class Solution {
public:
    void dfs(int i, int j, vector<vector<char>>& board, vector<int>& dr, vector<int>& dc){
        board[i][j]='*';
        for(int d=0; d<4; d++){
            int nr=i+dr[d];
            int nc=j+dc[d];
            if(nr>=0 && nr<board.size() && nc>=0 && nc<board[0].size() && board[nr][nc]=='O') dfs(nr, nc, board, dr, dc);
        }
    }
